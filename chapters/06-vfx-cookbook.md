...
## The stagecraft almanac — the scene-level shelf

Everything above happens *to a character*: a spark flies off the cube, a halo floats over it, a trail follows it. There is a second shelf of effects that happen to the **scene** — to the whole screen after it is drawn, to the light and what it cannot reach, to the ground where the bullet landed, to the weather, to the numbers in the corner, and to the sound. None of it is new machinery. Every card is one of the two mechanisms this chapter keeps repeating (*spawn small things with simple rules* or *bend colour with a tiny per-pixel program*) plus a third, quieter one that this shelf makes visible: **decide which layer, which event, or which clock the change rides on.** The **[stagecraft almanac](https://esorhizome.github.io/sparks-and-sprites/stagecraft.html)** holds 104 of them, A to Z four times, on one small stage, each a living loop plus a press reaction, each with a **rhyme** — the same effect with two or three dials turned. Eight families; each grows from a lesson already in the book.

**Screen & post-process** grows from [chapter 03](03-combining-vfx-with-sprites.md)'s shaders. A **shader** is a tiny program that answers "what colour is this pixel?" — and on a canvas that program can be written as a plain loop over the frame's bytes, which is what these thirteen cards do: **Crt** samples each pixel from a slightly bent position (`uv + uv·|uv|²·curve`) and darkens every other row; **Ordered** compares brightness against a 4×4 threshold tile and draws on/off — the Game Boy look; **Quantise** snaps colours to the nearest of a small palette; **Godrays** keeps only the bright pixels and smears them away from the sun. The Godot port is where the spelling honestly changes: the same arithmetic lives in thirteen `canvas_item` shaders reading the screen texture.

**Sprite & material shaders** is chapter 03's tint, outline and palette swap, aimed at the hero's own skin and given a state: **Frozen** maps brightness to a four-colour palette and sweeps it up from the feet; **Jelly** draws the sprite in horizontal strips and offsets each with a sine; **Xray** draws the part of the hero the wall hides as a flat colour on top of it; **Skewshadow** redraws the sprite sheared and squashed toward the ground.

**Weapons, impacts & decals** and **Particle mechanics** are this chapter's own sparks and waterdrops, grown up. The sparks recipe (four lines of physics) becomes **Impact**, where the hit's *material* picks the burst — sparks for metal, dust for stone, chips for wood, a ring for water; the waterdrops' "a particle's death is an event" becomes **Subemitter**, a particle that emits particles and spawns a burst when it dies; the trail becomes **Arc**, a live ribbon between a sword's last few positions. Around them sit the questions a shipped game must answer: where the stamps go when there are too many (**Decals**, a ring buffer), how a particle avoids allocating (**Pool**), which layer an effect draws in so it does not scroll twice (**Layers**), and what to drop when over budget (**Budget**).

**Water, weather & nature** grows from [chapter 04](04-backgrounds.md)'s **noise** (smooth organic randomness) and [chapter 16](16-depth-without-a-third-dimension.md)'s tides: **Cloudshadow** is a scrolling noise mask multiplied over the ground — the cheapest "outdoors" there is; **Wavesprings** is a row of springs, each pulled toward rest and toward its neighbours, so a splash becomes a wave that travels and settles; **Lens** is rain beading and running on the camera glass.

**Lighting & visibility** grows from chapter 16's candles and lanterns. The atlas drew light as a bright core plus a falloff; the almanac asks *what it cannot reach*: **Visibility** casts a ray to every wall corner and fills the fan; **Umbra** is a flickering point light plus that polygon, so pillars cast shadows that breathe; **Night** is a blue multiply layer with additive holes cut for lamps; **Fogofwar** remembers which tiles were seen. **Zap** (lightning) flashes the whole frame and is **opt-in** — a notice until clicked — as is **Muzzle** in the impacts family.

**UI & HUD feedback** grows from [chapter 12](12-cursors-and-living-buttons.md)'s living buttons and [chapter 13](13-text-that-moves.md)'s moving text: **Healthbar** snaps the red bar and lets a white ghost drain behind it over half a second, so the hit you just took is *visible*; **Odometer** tweens a score with rolling digits; **Bossbar** fills on the intro, shakes on hit and breaks a segment per phase.

**SFX & audio** grows from [chapter 07](07-sound-effects.md)'s three blips, and keeps its rule — no sound until the first click. Every card draws its mechanism (a meter, an envelope, a filter curve) so it is worth watching muted: **Footsteps** lets the surface under the foot pick the recipe; **Quiet** drops a lowpass cutoff underwater; **Jukebox** fades music layers in by intensity and waits for the bar to switch; **Lookahead** books notes slightly ahead on the audio clock instead of a frame timer, and shows the jitter it removes.

The one-page version — every card in one breath, a "wanted effect → card" table, the six load-bearing snippets and each engine's spelling per family — is the [stagecraft cheatsheet](../cheatsheets/stagecraft.md). The Godot project ports all 104 (+104 rhymes) in [`demos/godot/scenes/stage/`](../demos/godot/scenes/stage/) (menu key **L**), with the screen family as shaders in [`demos/godot/shaders/stage/`](../demos/godot/shaders/stage/).

▶ *See it:* [the stagecraft almanac](https://esorhizome.github.io/sparks-and-sprites/stagecraft.html) — press *Run all*, then click a running card to fire, poke, aim or scrub it.
