...
## Where the light goes next — the stagecraft almanac

The atlas drew a light as a bright core, a falloff, and the light thrown on things. The **[stagecraft almanac](https://esorhizome.github.io/sparks-and-sprites/stagecraft.html)**'s *Lighting & visibility* family asks the next question — *what can the light not reach?* **Visibility** casts a ray to every wall corner (and a hair either side), sorts the hits by angle and fills the fan: the lit region as a polygon. **Umbra** is the atlas's Candle placed in a room with pillars — the flicker plus that polygon, so the shadows breathe. Two cards extend this chapter's shadow row: **Skewshadow** redraws any sprite sheared and squashed toward the ground, the sun's angle setting the shear, and **Blobshadow** is the contact shadow on a slope — a ground raycast under the hero, tilted to the surface, shrinking with height. **Zap** (lightning) is opt-in, exactly like Xenon below. One page: the [stagecraft cheatsheet](../cheatsheets/stagecraft.md).
