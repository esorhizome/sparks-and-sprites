extends Node2D
## THE GLYPH GRIMOIRE — one phrase, 104 text effects, in GDScript.
## The full port of the web page (docs/text-fx.html): fourteen effect
## families, one page per family (←/→ turns pages), every card a little
## stage where the phrase "just this" arrives, breathes, glows, scrambles,
## or misbehaves. Click a card to trigger its press reaction — some
## effects aim at where you click. Esc = menu.
##
## The effects live in scenes/textfx/ — one file per family, each effect
## ~30 lines of init/tick/press/draw. scenes/textfx/kit.gd owns the
## phrase: its per-letter layout, its glow, its wrong glyphs.
##
## RIGHT-CLICK a card to hear its RHYME: the same effect with two or
## three dials turned (scenes/textfx/*_r.gd). Each rhyme file preloads
## its original as Base and overrides only the branches whose dials
## moved — diffing original against rhyme IS the lesson.
...
const COLS := 4
const CELL := Vector2(232, 200)
const STAGE := Vector2(220, 132)
...
## freshly seeded state owned by whichever file has it now.
...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(14, 24),
		"THE GLYPH GRIMOIRE — %d text effects (+%d rhymes) — %s (%d/%d): %s" %
	...
	draw_string(ThemeDB.fallback_font, Vector2(14, 42),
		"←/→ turn the page · click a card to trigger it · RIGHT-CLICK to hear its rhyme · Esc = menu",
