extends Node2D
## THE WORLD WORKSHOP — 13 generators (+13 rhymes), in GDScript. The full
## port of the web page (docs/worlds.html): procedural generation — points
## that keep their distance, a ground line from layered noise, regions by
## nearest seed, trees from a rewrite rule, caves grown by a neighbour rule,
## rooms carved by halving, mazes with two personalities, tiles chosen by
## constraint, tiles picked by a bitmask, weighted loot and fair bags, a
## spawn director's curve, a versioned save, and one seed for everything.
## Chapter 19 in the book.
##
## Three families, one page of eight cards at a time (←/→ turns pages).
## Click a card to regenerate, place, paint or roll — DRAG the cards that
## say so. RIGHT-CLICK a card to see its RHYME. DOUBLE-CLICK to enlarge.
## 1 / 2 / 4 set the tempo. Esc = menu.
##
## The generators live in scenes/world/ — one file per family, each card an
## init/press/tick/draw quartet over a card dictionary. scenes/world/kit.gd
## is the lexicon's kit plus grids, 2-D noise and a shuffle.
##
...
const FAMILIES := [
	preload("res://scenes/world/scatter.gd"),
	preload("res://scenes/world/rooms.gd"),
	preload("res://scenes/world/dice.gd"),
]
...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(14, 24),
		"THE WORLD WORKSHOP — %d generators (+%d rhymes) — %s%s · page %d/%d · tempo ×%d" %
	...
	if big.is_empty():
		draw_string(ThemeDB.fallback_font, Vector2(14, 530),
			"←/→ page · click a card to regenerate or place (DRAG the ones that say so) · RIGHT-CLICK = rhyme · DOUBLE-CLICK = enlarge · 1/2/4 tempo · Esc = menu",
