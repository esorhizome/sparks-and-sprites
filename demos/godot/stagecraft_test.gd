extends SceneTree
## Headless test for the full stagecraft almanac (not a demo).
...
## Run: godot --headless --path . -s res://stagecraft_test.gd

const Lex := preload("res://scenes/stagecraft.gd")
...
func _initialize() -> void:
	...
	assert(total == 104, "expected 104 cards, found %d" % total)
	assert(letters.size() == 26, "the alphabet has a gap: %d distinct letters" % letters.size())
	for l in letters:
		assert(letters[l] == 4, "letter %s owns %d cards, wanted exactly 4" % [l, letters[l]])
	print("stagecraft logic pass: %d styles + %d rhymes ticked and pressed, A to Z four times, families %s" % [total, total, str(sizes)])
	change_scene_to_file("res://scenes/stagecraft.tscn")
...
func _tick() -> void:
	...
	if frame % 12 == 0:
		...
		page += 1
		if page > 17:                                     # window pages + wrap slack
			print("STAGECRAFT TEST COMPLETE — %d styles + %d rhymes, all pages drawn, clicked, dragged, rhymed and enlarged" % [total, total])
			quit()
