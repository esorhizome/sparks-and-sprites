extends SceneTree
## Headless test for the full world workshop (not a demo).
...
## Run: godot --headless --path . -s res://worlds_test.gd

const Lex := preload("res://scenes/worlds.gd")
...
func _initialize() -> void:
	...
	assert(total == 13, "expected 13 cards, found %d" % total)
	print("worlds logic pass: %d styles + %d rhymes ticked and pressed, families %s" % [total, total, str(sizes)])
	change_scene_to_file("res://scenes/worlds.tscn")
...
func _tick() -> void:
	...
	if frame % 12 == 0:
		...
		page += 1
		if page > 3:                                     # window pages + wrap slack
			print("WORLDS TEST COMPLETE — %d styles + %d rhymes, all pages drawn, clicked, dragged, rhymed and enlarged" % [total, total])
			quit()
